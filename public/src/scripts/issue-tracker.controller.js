(function (angular) {
    ...
    function IssueTrackerCtrl($scope, $uibModal, $log, $http, $q, snakeService, issuesService, issueEditService, issueAddService, commentAddService, helpService, settingsService, dialogService) {
        ...
        vm.log = true;
        vm.updIssueList = {};
        vm.isAutorized = false;
        vm.errorMsg = false;
        $http.get("./src/json/issues.json").then(function (response) {
            vm.issueList = response.data;
            $log.info(response.data);
            vm.initIssue = vm.copyIssue();
        });
        $http.get("./src/json/comments.json").then(function (response) {
            vm.comments = response.data;
        });
        $http.get("./src/json/poll.json").then(function (response) {
            vm.poll = response.data;
        });
        $http.get("./src/json/snake.json").then(function (response) {
            vm.snake = response.data;
        });
        $http.get("./src/json/credentials.json").then(function (response) {
        ...
        vm.register = function () {
            if (vm.regPassword !== '') {
                ...
                console.log(vm.reglogin);
                vm.credentials.push(vm.credents);
                $http.post("./credentials", vm.credentials).then(function (response) {});
            }
        };
        vm.checkCredentials = function () {
            angular.forEach(vm.credentials, function (cred) {
                if (cred.login == vm.login && cred.password == vm.password) {
                    vm.isAutorized = true;
                    vm.errorMsg = false;
                }
                else {
        ...
        vm.getSnakeModal = function () {
            var modalInstance = snakeService.create(vm.snake, vm.login);
            modalInstance.result.then(function (count) {
                if (count !== null) {}
            });
        };
